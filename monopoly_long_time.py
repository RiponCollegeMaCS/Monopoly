...
class Player:
    ...
    def pay_out_of_jail(self, game_info):
        if self.chance_card:
            game_info.getting_out_of_jail['Chance Card'] += 1
            ...
            game_info.getting_out_of_jail['Community Chest Card'] += 1
            ...
            game_info.getting_out_of_jail['Paid'] += 1
...
class Game:
    def __init__(self, list_of_players, auctions_enabled=True, free_parking_pool=False,
        ...
        self.loss_reason = "Unknown."  # To store how a player lost the game.
        self.going_to_jail = {'Go To Jail': 0, 'Chance Card': 0, 'Community Chest Card': 0, 'Speeding': 0}
        self.getting_out_of_jail = {'Doubles': 0, 'Chance Card': 0, 'Community Chest Card': 0, 'Paid': 0}
    ...
    def community_chest(self, player):
        ...
        if card == 1:  # GET OUT OF JAIL FREE
            ...
            self.exchange_money(amount=50, giver=player, receiver=self.free_parking, summary="CC2")
            ...
            for individual in self.active_players:  # For each player...
                self.exchange_money(amount=10, giver=individual, receiver=player, summary="CC3")
        elif card == 4:  # XMAS FUND MATURES / COLLECT $100
            self.exchange_money(amount=100, giver=self.bank, receiver=player, summary="CC4")
        elif card == 5:  # INCOME TAX REFUND / COLLECT $20
            self.exchange_money(amount=20, giver=self.bank, receiver=player, summary="CC5")
        elif card == 6:  # YOU INHERIT $100
            self.exchange_money(amount=100, giver=self.bank, receiver=player, summary="CC6")
        elif card == 7:  # YOU HAVE WON SECOND PRIZE IN A BEAUTY CONTEST / COLLECT $10
            self.exchange_money(amount=10, giver=self.bank, receiver=player, summary="CC7")
        elif card == 8:  # BANK ERROR IN YOUR FAVOR / COLLECT $200
            self.exchange_money(amount=200, giver=self.bank, receiver=player, summary="CC8")
        elif card == 9:  # RECEIVE $25 / CONSULTANCY FEE [WORDING UPDATED IN 2008]
            self.exchange_money(amount=25, giver=self.bank, receiver=player, summary="CC9")
            ...
            self.exchange_money(amount=100, giver=self.bank, receiver=player, summary="CC12")
        elif card == 13:  # DOCTOR'S FEE / PAY $50
            self.exchange_money(amount=50, giver=player, receiver=self.free_parking, summary="CC13")
        elif card == 14:  # FROM SALE OF STOCK / YOU GET $50 [UPDATED IN 2008]
            self.exchange_money(amount=50, giver=self.bank, receiver=player, summary="CC14")
        elif card == 15:  # PAY HOSPITAL $100
            self.exchange_money(amount=100, giver=player, receiver=self.free_parking, summary="CC15")
        elif card == 16:  # GO TO JAIL
            self.going_to_jail['Community Chest Card'] += 1
    ...
    def chance(self, player):
        ...
        if card == 1:  # GET OUT OF JAIL FREE
            ...
            self.going_to_jail['Chance Card'] += 1
            self.go_to_jail(player)  # Send player to jail.
        elif card == 3:  # YOUR BUILDING LOAN MATURES / COLLECT $150
            self.exchange_money(amount=150, giver=self.bank, receiver=player, summary="C3")
            ...
            self.exchange_money(amount=15, giver=player, receiver=self.free_parking, summary="CC12")
            ...
            for individual in self.active_players:  # For each player...
                self.exchange_money(amount=50, giver=player, receiver=individual, summary="C15")
        elif card == 16:  # BANK PAYS YOU DIVIDEND OF $50
            self.exchange_money(amount=50, giver=self.bank, receiver=player, summary="C16")
    ...
    def pay_rent(self, player):
        ...
        # Pay the rent.
        summary = "Paying rent.", current_property.name, current_property.group, current_property.buildings
    ...
    def board_action(self, player, board_space):
        if board_space.name == "Just Visiting / In Jail":
            ...
            self.exchange_money(amount=200, giver=player, receiver=self.free_parking, summary="Paying Income Tax.")
            ...
            self.going_to_jail['Go To Jail'] += 1
            self.go_to_jail(player)  # The player goes to jail.

        elif board_space.name == "Luxury Tax":
            # The player pays a $100 tax.
            self.exchange_money(amount=100, giver=player, receiver=self.free_parking, summary="Paying Luxury Tax.")
        ...
        if player.in_jail:  # Player is in jail.
            ...
            if player.jail_decision(self):
                ...
                if die1 == die2:  # The player rolled doubles.
                    self.getting_out_of_jail['Doubles'] += 1
                    player.jail_counter = 0  # Reset the jail counter.
    ...
    def play(self):
        ...
        if len(self.active_players) == 1:
            ...
            self.winner = 0
            self.loss_reason = "Tie game."

        # Ending report.
        # results = [self.winner, self.turn_counter]
        # results.extend(self.loss_reason)
        results = []
        for board_space in self.board:
            results.append(board_space.visits)

        for keys in self.going_to_jail:
            print([keys, self.going_to_jail[keys]])
        for keys in self.getting_out_of_jail:
            print([keys, self.getting_out_of_jail[keys]])

        return results


def board_space_dist():
