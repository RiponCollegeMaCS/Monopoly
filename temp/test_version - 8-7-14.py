...
class Player:
    ...
    def __init__(self, number, max_houses=5, jail_time=3, buying_threshold=500,
                 smart_jail_strategy=False, complete_monopoly=False,
                 group_preferences=({"Brown": False,
                                     "Light Blue": False,
                                     "Pink": False,
                                     "Orange": False,
                                     "Red": False,
...
class Game:
    ...
    def auction(self, board_space):
        ...
        for current_player in self.players:
            if current_player.group_preferences[board_space.group]:
        ...
        if board_space.name in ["Go", "Just Visiting / In Jail"]:
            ...
            if board_space in player.inventory:
                ...
                if self.trip_to_start == True and player.passed_go == False:
                    ...
                    elif player.group_preferences[board_space.group] and player.money - board_space.price > 0:
                        self.buy_property(player, board_space)
    ...
    def update_status(self):
        ...
        if self.turn_counter == 300:
            ...
            if player1_assets > player2_assets:
                self.result = 1
            elif player2_assets > player1_assets:
                self.result = 2
            else:
...
def generate_random_opponent():
    group_preferences_dict = ({"Brown": choice([True, False]),
                               "Light Blue": choice([True, False]),
                               "Pink": choice([True, False]),
                               "Orange": choice([True, False]),
                               "Red": choice([True, False]),
                               "Yellow": choice([True, False]),
                               "Green": choice([True, False]),
                               "Dark Blue": choice([True, False]),
                               "Railroad": choice([True, False]),
                               "Utility": choice([True, False])})

    list_of_groups = ["Brown", "Light Blue", "Pink", "Orange", "Red", "Yellow", "Green", "Dark Blue",
                      "Railroad", "Utility"]

    preference_groups = sample(list_of_groups, choice([0, 1, 2, 3]))

    for group_name in list_of_groups:
        if group_name in preference_groups:
            group_preferences_dict[group_name] = True
    ...
    player = Player(2, buying_threshold=randint(1, 1001),
                    ...
                    group_preferences=group_preferences_dict)
...
def success_indicator(base_player, number_of_games=1000, procs=2):
    ...
    if __name__ == '__main__':  # Technicality
        proc_list = [
            Process(target=play_set, args=(base_player, int(number_of_games / procs), q_list[i])) for i
            in
...
def main():
    # test_proc()
    player1 = Player(1)
    success_rate = success_indicator(base_player=player1, number_of_games=1000, procs=4)
    print(success_rate)
