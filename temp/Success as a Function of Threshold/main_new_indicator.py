...
def play_set(strategy, number_of_games, results_q):
    for i in range(number_of_games):
        ...
        results_q.put(current_game_result[0])  # Store the game's result in the Queue provided.
    #print(results_list)
...
def success_indicator(strategy, number_of_games=1000, procs=2):
    ...
    if __name__ == '__main__': #Technicality
        proc_list = [Process(target=play_set, args=(strategy, int(number_of_games/procs), q_list[i])) for i in range(procs)] #List of processes.
...
def simple_success_indicator(strategy, number_of_games=1000):
    results_list = []
    for i in range(number_of_games):
        # Create players.
        base_player = Player(1, [strategy])  # The main player.
...
def generate_confidence_interval():
    ...
    for i in range(50):
        success_rate = success_indicator(player1_threshold, 1000)
        print(success_rate)

# main
def main():
    player1_threshold = 100
    #success_rate = simple_success_indicator(player1_threshold, 1000)
    success_rate = success_indicator(player1_threshold, 1000, 2)
    print(success_rate)
