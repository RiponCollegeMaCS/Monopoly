...
class Player:
    def __init__(self,
        ...
        self.wish_list = [] #Keep track of groups that are one short of a monopoly (for trades)
        self.negative_group_counts = {"Brown": -2, "Light Blue": -3, "Pink": -3, "Orange": -3,
    ...
    def reset_values(self):
        ...
        self.negative_group_counts = {"Brown": -2, "Light Blue": -3, "Pink": -3, "Orange": -3,
                              "Red": -3, "Yellow": -3, "Green": -3, "Dark Blue": -2,
                              "Utility": -2, "Railroad": -4} #Number of properties from monopoly        
        ...
        for player in self.active_players:
            if player.initial_inventory:
                for id in player.initial_inventory:
                    self.update_inventories(player_from="Bank", player_to=player, prop_list=[self.board[id]])
                ...
                if current_party.money <= 0:
                    ...
                    if len(self.active_players) > 1:
                        ...
                        if other_party == self.bank:
                            ...
                            self.update_inventories(player_from=current_party, player_to=other_party, prop_list=current_party.inventory)
#                            other_party.inventory.extend(current_party.inventory)
    ...
    #Trading based on group rankings.  Only 1-1 trades are allowed, and trades only happen 
    #if both players gain a complete monopoly.
    def ranking_trading(self, purchase_player, prop):
        #Find the oppononent.        
        for player in self.active_players:
            if player != purchase_player:
                opponent = player
        if purchase_player.negative_group_counts[prop[group]] != 0: #Make sure we aren't breaking a monopoly for purchase_player
            if opponent.negative_group_counts[prop[group]] == -1:
                for group in purchase_player.wishlist:
                    if purchase_player.group_ordering
    ...
    def update_inventories(self, player_from, player_to, prop_list):
        if player_from == "Bank":
            ...
            for prop in prop_list: 
                ...
                player_to.negative_group_counts[prop.group] += 1
                if player_to.negative_group_counts[prop.group] == -1:
                    player_to.wish_list.append(prop.group)                    
        else:
            for prop in prop_list:
                #Update inventory, group count, and wish_list for player_from
                player_from.inventory.remove(prop)
                player_from.negative_group_counts[prop.group] -= 1
                if player_to.negative_group_counts[prop.group] == -2:
                    player_from.wish_list.remove(prop.group)
                                       
                #Update inventory, group count, and wish_list for player_to
                player_to.inventory.append(prop)
                player_to.negative_group_counts[prop.group] += 1
                if player_to.negative_group_counts[prop.group] == -1:
                    player_to.wish_list.append(prop.group)            
