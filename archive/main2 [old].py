...
def one_dim_climb():
    new_strategy = 50  # randint(1, 500)  # Initial strategy.
    perturbations = [-1, 1]  # Possible perturbations.
    ...
    for series in range(100):
        ...
        for i in range(1000):
            # Create players.
            base_player = Player(1, [new_strategy])  # The main player.
            opponent = Player(2, [randint(1, 500)])  # The random opponent.
        ...
        if series > 0:  # Not the first time.
            ...
            if new_success_indicator > old_success_indicator:
                new_strategy = new_strategy + choice(perturbations)
...
def produce_pairs():
    with open('pairs3.csv', 'w', newline='') as csvfile:
        output_file = csv.writer(csvfile, quotechar=',')

        for buying_threshold in range(5, 251, 5):
            current_game_series_results = []
            for game_series in range(2000):
                # Create players.
                base_player = Player(1, [buying_threshold])  # The main player.
                opponent = Player(2, [250])  # The random opponent.
...
def produce_pairs_opp_dist():
    with open('pairs.csv', 'w', newline='') as csvfile:
        output_file = csv.writer(csvfile, quotechar=',')

        for buying_threshold in range(1, 101, 1):
            current_game_series_results = []
            for i in range(0, 10000):
                # for opponent_threshold in range(1, 251):
                # Create players.
                base_player = Player(1, [buying_threshold])  # The main player.
                opponent_threshold = randint(1, 250)
                opponent = Player(2, [opponent_threshold])  # The opponent.
