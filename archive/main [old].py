...
# Import the randint and shuffle functions.
from random import shuffle, randint

# Import the Decimal module for better rounding.
from decimal import *
...
class Player:
    def __init__(self, number, buying_threshold, development_threshold, jail_time):
        self.number = number
        ...
        self.buying_threshold = buying_threshold  # The player's strategy parameter.
...
class Game:
    def __init__(self, number_of_players):
        ...
        self.players = [Player(i, 200, 100, 3) for i in range(1, number_of_players + 1)]  # Create  a list of players.
    ...
    # Creates aa BoardLocation object for each space on the board.
    ...
    def community_chest(self, player):
        ...
        print(player.number, "drew", "CC card", card)
    ...
    def chance(self, player):
        ...
        print(player.number, "drew", "C card", card)
        if card == 1:  # GET OUT OF JAIL FREE
    ...
    def move_ahead(self, player, number_of_spaces):
        ...
        if new_position < player.position:  # Does the player pass Go?
            ...
            print("player", player.number, "passed Go")
    ...
    def move_to(self, player, new_position):
        if new_position < player.position:  # Does the player pass Go?
            self.change_money(player, 200)  # The player collects $200 for passing Go.
            print("player", player.number, "passed Go with card")
        player.position = new_position  # Update the player's position.
        self.board[new_position].visits += 1  # Increase hit counter.

    # Sends a player to jail.
    def go_to_jail(self, player):
        print("player", player.number, "is in jail")
    ...
    def buy_property(self, player, board_space):
        ...
        print("player", board_space.owner, "bought", board_space.name)
    ...
    def pay_rent(self, player, diceroll):
        ...
        # Rent for Utilities.
            ...
            if self.board[12].owner == owner.number and self.board[28].owner == owner.number:
        ...
        # Rent for color-group properties.
            ...
            if current_property.buildings == 5:  # Check to see if there is a hotel.
                ...
                if current_property.group in player.monopolies:  # If the player has a monopoly...
        ...
        # A temp. report.
        print("player", player.number, "paid $", rent, "to", owner.number)
    ...
    def sell_building(self, player, property, building):
        if building == "house":
            ...
            if property.buildings == 5:
                ...
                property.buildings = 0
        print(player.number, "sold", building, "on", property.name)
    ...
    def change_money(self, player, increment):
        ...
        if player.money <= 0:
            ...
            # Mortgage properties.
            for board_space in self.board:  # Cycle through all board spaces.
                if board_space.buildings > 0 and board_space.owner == player.number:
                    print("!!!!!", board_space.name, "has", board_space.buildings, "buildings")
                if board_space.owner == player.number and board_space.buildings == 0:
                    ...
                    print(board_space.name, "mortgaged")
                    if player.money > 0:  # The player is out of the hole.
        ...
        for board_space in self.board:  # Cycle through all board spaces.
            if board_space.owner == -player.number:
                ...
                if player.money - unmortgage_price >= player.buying_threshold:
                    ...
                    if self.monopoly_status(player, board_space) == True:
                        player.monopolies.append(board_space.group)
                    print(board_space.name, "unmortgaged")
                else:
                        ...
                        if player.money - board_space.house_cost >= player.development_threshold:  # Strategy param.
                            if self.even_building_test(board_space):  # Building "evenly".
                                ...
                                if building_supply > 0:  # There's a building available.
                                    ...
                                    print(building, "bought for", board_space.name, "by player", player.number)
                                    keep_building = True
        ...
        if board_space.name in ["Go", "Just Visiting / In Jail", "Free Parking"]:
            ...
            if board_space.owner == player.number:
                ...
                if player.money - board_space.price >= player.buying_threshold:
                    ...
                    print("player", player.number, "put", board_space.name, "up for auction")
    ...
    def take_turn(self, player):
        ...
        if player.in_jail == True:  # Player is in jail.
        ...
        # The main loop.
        while self.move_again == True and player.money > 0:
            ...
            # Temp. report.
            print("player", player.number, "rolled", die1, die2)
            ...
            # Temp. report.
            print("player", player.number, "| money: $", player.money, "| location:", board_space.name)
            print(player.monopolies)
    ...
    def update_status(self):
        for single_player in self.players:
            if single_player.money <= 0:
                ...
                print("player", single_player.number, "lost")

        if self.turn_counter == 1000:
            self.game_status = "done"
            print("game was capped")
    ...
    def play(self):
        ...
        # Shuffle the players.
        playing_order = [i for i in range(1, self.number_of_players + 1)]
        ...
        while self.game_status == "live":
            ...
            report = []
            for board_space in self.board:
                report.append(board_space.owner)
            print(report)
            report2 = []
            for individual_player in self.players:
                report2.append(individual_player.money)
            print("money totals:", report2)
        print(". . .the game is done. It lasted", self.turn_counter, "turns.")  # Ending report.
        input("Pause...")


# main
def main():
    game1 = Game(2)  # Create a game object.
