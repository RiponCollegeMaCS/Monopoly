...
class Player:
    def __init__(self, number, development_threshold=100, max_houses=5, jail_time=3,
                 smart_jail_strategy=False, complete_monopoly=True,
                 group_thresholds=({"Brown": 100,
                                    "Light Blue": 100,
                                    "Pink": 100,
                                    "Orange": 100,
                                    "Red": 100,
...
class Game:
    ...
    def auction(self, board_space):
        ...
        for current_player in self.players:
            current_player.auction_bid = current_player.money - current_player.group_thresholds[board_space.group]
        ...
        if board_space.name in ["Go", "Just Visiting / In Jail"]:
            ...
            if board_space.owner == player.number:
                ...
                if self.trip_to_start and player.passed_go == False:
                    ...
                    elif player.money - board_space.price >= player.group_thresholds[board_space.group]:
                        self.buy_property(player, board_space)
                    else:
                        board_space.owner = 0
                        if self.auctions:
    ...
    def play(self):
        ...
        # Ending report.
        return [self.result, self.turn_counter, self.early_winner]
...
def play_game_series(number_of_games=100, leap=50):
    print("Playing...")
...
def play_set(strategy, number_of_games, results_q):
    for i in range(number_of_games):
        # Create players.
        base_player = Player(1, strategy)  # The main player.
        opponent = Player(2, randint(1, 1000))  # The random opponent.
...
def success_indicator(strategy, number_of_games=1000, procs=2):
    q_list = [Queue() for i in range(procs)]
    if __name__ == '__main__':
        proc_list = [Process(target=play_set, args=(strategy, int(number_of_games / procs), q_list[i]))
        ...
        print("results = " + str(results_list))
        print("results length = " + str(len(results_list)))
        success_rate = results_list.count(1) / number_of_games
        return success_rate


def test_proc():
    success_indicator(100, procs=2)
